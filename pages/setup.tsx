...
const Setup: NextPage = () => {
    ...
    const initGame = async () => {
        ...
        setRoomId(topic);
        setRoomUri(`http://localhost:3000/setup?id=${libp2p.peerId.toString()}&room=${topic}`)

        setIsInit(true)
    }

    useEffect(() => {
        if (router.query && router.query.id && router.query.room && !roomId && !peerId && !isInit) {
    ...
    const sendMessage = () => {
        libp2p.pubsub.publish(roomId, uint8ArrayFromString('Sending a test message!')).catch((err : any) => {
            console.error(err)
        })
    }
